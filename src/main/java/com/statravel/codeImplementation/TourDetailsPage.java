...
import com.statravel.ttcApi.util.ItineraryDay;
...
public class TourDetailsPage extends BaseUtil {
	...
	public List<ItineraryDay> getItineraryDays() {
		...
		List<ItineraryDay> itineraryDay = labelsDayTitle
				.stream().map(
						it -> new ItineraryDay(
								StringUtils.substringAfter(it.findElements(By.xpath(labelDayTitleXpathRelative)).get(0)
										.getText(), "— \n"),
								StringUtils.substringBefore(it.findElements(By.xpath(labelDayDescrXpathRelative)).get(0).getText(), "\n"),
								it.findElements(By.xpath(labelDayMealsXpathRelative)).size() > 0 ? Arrays.asList(WordUtils
										.capitalize(StringUtils.substringAfter(
												it.findElements(By.xpath(labelDayMealsXpathRelative)).get(0).getText().toLowerCase(), ": "))
	...
	public void verifyTourDetails(CheapestTour expTour) {
		...
		if (expTour.getFullPrice() != expTour.getDiscountedPrice()) {
			...
			softAssertion.assertTrue(driver.findElements(By.xpath(labelWasPriceXpath)).size() == 0, "WasPrice label is found");
			softAssertion.assertTrue(driver.findElements(By.xpath(labelDiscountXpath)).size() == 0, "Discount % label is found");
		}
		softAssertion.assertEquals(
				LocalDate.parse(labelDate.getText().replaceAll("[a-zA-Z][ ]*", ""), DateTimeFormatter.ofPattern("dd/MM/yyyy")),
				LocalDate.parse(expTour.getDeparture().getOperatingStartDate()), "StartDate verification failed");

		Map<String, List<String>> incl = labelIncludedTitle.stream().collect(Collectors.toMap(t -> t.getText(),
				t -> t.findElements(By.xpath(labelIncludedXpathRelative)).stream().map(tt -> tt.getText()).collect(Collectors.toList())));
		...
		softAssertion.assertEquals(labelDescription.getText(), expTour.getContent().getDescription(), "Description verification failed");
		// start, end city
		...
		softAssertion.assertEquals(Integer.parseInt(labelRouteDuration.getText().replaceAll("[^0-9]", "")), expTour.getDuration(),
				"RouteDuration verification failed");
